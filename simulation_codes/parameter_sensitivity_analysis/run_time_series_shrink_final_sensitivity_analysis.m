function run_result=run_time_series_shrink_final_sensitivity_analysis(p,dt,antigen_satu,tau_up,init_list,T_list_temp,K2,func_num,checked_parameter_list,check_ind)
...
mTh0=init_list(3);
mTr0=init_list(4);
Th_naive0=init_list(5);
Tr_naive0=init_list(6);
Th_memory0=init_list(7);
Tr_memory0=init_list(8);
R0=init_list(9);
...
checked_parameter_num=check_ind(1);
check_ratio=check_ind(2);

conduct_change=[convertContainedStringsToChars(checked_parameter_list(checked_parameter_num)),'=',convertContainedStringsToChars(checked_parameter_list(checked_parameter_num)),'*',num2str(check_ratio)];
eval(conduct_change);
