function fig=figS1_plot_time_series(history_result_linear_former,history_result_linear_latter,history_result_sig_former,history_result_sig_latter,history_result_step_former,history_result_step_latter,func_num_mTr)
...
%step
  ...
  %intensity
   ...
   xlim([min(T_latter_linear),max(T_latter_linear)])
   ylim([0,3000])
   sgtitle(['Th:linear,Tr:',Tr_response])
   ax=gca;
   ax.FontSize=20;
...
% %    
   ...
   xlim([min(T_latter_sig),max(T_latter_sig)])
   ylim([0,3000])
   sgtitle(['Th:sigmoid,Tr:',Tr_response])
   ax=gca;
   ax.FontSize=20;
...
% %     saveas(f,['fig6_dose_response_sig_',num2str(DateString),').eps'],'epsc2')%最後に"epsc2"を付けないとモノクロで出力されてしまう
   ...
   xlim([min(T_latter_step),max(T_latter_step)])
   ylim([0,3000])
   sgtitle(['Th:step,Tr:',Tr_response])
   ax=gca;
   ax.FontSize=20;
